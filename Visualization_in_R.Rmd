...
# On the first row we determine that we are using the dataset suhteet (relationships) and the variables PARISUHDE (intimate relationship) and SEKS_AKTIIVISUUS (sexual activity). geom="bar" determines that we want a dot plot.
# On the second row we define parameters for how the dots appear in the plot.
# On the third row we determine the headings.
...
plot3 <- qplot(data=suhteet, PARISUHDE, SEKS_AKTIIVISUUS, geom="dotplot", 
            ...
            xlab="Parisuhteen tyyppi", ylab="Seksuaalinen aktiivisuus") + # Define names for the x and y axis.
  theme_light()

# Again on the first row we define the data and variables, as well as the plot type (violin) and fill=PARISUHDE determines that the color filling is done based on this variable.
# scale_fill_discrete() is used to determine the coloring. In this case it's a discrete palette which is suitable for discerete variables. There are loads of different scale_fills, just type scale_fill, press tab and have a look.
...
grid.arrange(plot3, plot4, ncol=2) # This function comes from the package gridExtra. It does something similar that the par() did in the example above, but this need to be used after creating/assigning the images and then the plot names are called here. ncol=2 determines the number of columns. The plots won't appear before this function is used!
